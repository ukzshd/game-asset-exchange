...
async function initTables(db) {
    await db.exec(`
        ...
        CREATE TABLE IF NOT EXISTS products (
          ...
          spu_id INTEGER,
          sku_id INTEGER,
          catalog_source TEXT DEFAULT 'platform',
        ...
        CREATE TABLE IF NOT EXISTS orders (
          ...
          seller_gross_amount DOUBLE PRECISION DEFAULT 0,
          seller_net_amount DOUBLE PRECISION DEFAULT 0,
          settlement_status TEXT DEFAULT '',
          buyer_confirmed_at TIMESTAMPTZ,
          delivered_by_seller_at TIMESTAMPTZ,
        ...
        CREATE INDEX IF NOT EXISTS idx_products_source ON products(catalog_source, listing_status);
        CREATE INDEX IF NOT EXISTS idx_products_seller ON products(seller_user_id);
        CREATE INDEX IF NOT EXISTS idx_orders_user ON orders(user_id);
        CREATE INDEX IF NOT EXISTS idx_orders_status ON orders(status);
        CREATE INDEX IF NOT EXISTS idx_orders_source ON orders(order_source, seller_user_id);
        ...
        CREATE INDEX IF NOT EXISTS idx_order_inventory_reservations_order ON order_inventory_reservations(order_id);
        CREATE INDEX IF NOT EXISTS idx_order_inventory_reservations_lot ON order_inventory_reservations(lot_id);
...
async function migrateSchema(db) {
    await db.exec(`
        ...
        ALTER TABLE products ADD COLUMN IF NOT EXISTS catalog_source TEXT DEFAULT 'platform';
        ALTER TABLE products ADD COLUMN IF NOT EXISTS seller_user_id INTEGER REFERENCES users(id) ON DELETE SET NULL;
        ...
        ALTER TABLE product_spus ADD COLUMN IF NOT EXISTS catalog_source TEXT DEFAULT 'platform';
        ALTER TABLE product_spus ADD COLUMN IF NOT EXISTS seller_user_id INTEGER REFERENCES users(id) ON DELETE SET NULL;
        ALTER TABLE product_spus ADD COLUMN IF NOT EXISTS listing_status TEXT DEFAULT 'published';

        ALTER TABLE product_skus ADD COLUMN IF NOT EXISTS catalog_source TEXT DEFAULT 'platform';
        ALTER TABLE product_skus ADD COLUMN IF NOT EXISTS seller_user_id INTEGER REFERENCES users(id) ON DELETE SET NULL;
        ALTER TABLE product_skus ADD COLUMN IF NOT EXISTS listing_status TEXT DEFAULT 'published';

        ALTER TABLE inventory_lots ADD COLUMN IF NOT EXISTS owner_user_id INTEGER REFERENCES users(id) ON DELETE SET NULL;
        ...
        ALTER TABLE orders ADD COLUMN IF NOT EXISTS order_source TEXT DEFAULT 'platform';
        ALTER TABLE orders ADD COLUMN IF NOT EXISTS seller_user_id INTEGER REFERENCES users(id) ON DELETE SET NULL;
        ALTER TABLE orders ADD COLUMN IF NOT EXISTS platform_fee_rate DOUBLE PRECISION DEFAULT 0;
        ALTER TABLE orders ADD COLUMN IF NOT EXISTS platform_fee_amount DOUBLE PRECISION DEFAULT 0;
        ALTER TABLE orders ADD COLUMN IF NOT EXISTS seller_gross_amount DOUBLE PRECISION DEFAULT 0;
        ALTER TABLE orders ADD COLUMN IF NOT EXISTS seller_net_amount DOUBLE PRECISION DEFAULT 0;
        ALTER TABLE orders ADD COLUMN IF NOT EXISTS settlement_status TEXT DEFAULT '';
        ALTER TABLE orders ADD COLUMN IF NOT EXISTS buyer_confirmed_at TIMESTAMPTZ;
        ALTER TABLE orders ADD COLUMN IF NOT EXISTS delivered_by_seller_at TIMESTAMPTZ;
        ALTER TABLE orders ADD COLUMN IF NOT EXISTS auto_complete_at TIMESTAMPTZ;
        ALTER TABLE orders ADD COLUMN IF NOT EXISTS dispute_status TEXT DEFAULT '';
    ...
    `);
    await initMarketplaceTables(db);

    if (resolveDatabaseUrl() !== 'pg-mem') {
        await db.exec(`
            CREATE TABLE IF NOT EXISTS order_inventory_reservations (
              id SERIAL PRIMARY KEY,
              order_id INTEGER NOT NULL,
    ...
    await db.exec(`
        ...
        UPDATE orders SET order_source = 'platform' WHERE order_source IS NULL OR order_source = '';
        UPDATE orders SET platform_fee_rate = 0 WHERE platform_fee_rate IS NULL;
        UPDATE orders SET platform_fee_amount = 0 WHERE platform_fee_amount IS NULL;
        UPDATE orders SET seller_gross_amount = 0 WHERE seller_gross_amount IS NULL;
        UPDATE orders SET seller_net_amount = 0 WHERE seller_net_amount IS NULL;
        UPDATE orders SET settlement_status = '' WHERE settlement_status IS NULL;
        UPDATE orders SET dispute_status = '' WHERE dispute_status IS NULL;
        ...
        UPDATE products SET catalog_source = 'platform' WHERE catalog_source IS NULL OR catalog_source = '';
        UPDATE products SET listing_status = 'published' WHERE listing_status IS NULL OR listing_status = '';
        UPDATE product_spus SET catalog_source = 'platform' WHERE catalog_source IS NULL OR catalog_source = '';
        UPDATE product_spus SET listing_status = 'published' WHERE listing_status IS NULL OR listing_status = '';
        UPDATE product_skus SET catalog_source = 'platform' WHERE catalog_source IS NULL OR catalog_source = '';
        UPDATE product_skus SET listing_status = 'published' WHERE listing_status IS NULL OR listing_status = '';
    `);
...
async function initMarketplaceTables(db) {
    if (resolveDatabaseUrl() === 'pg-mem') {
        await db.exec(`
    ...
    await db.exec(`
        ...
        CREATE UNIQUE INDEX IF NOT EXISTS idx_seller_profiles_user ON seller_profiles(user_id);
        CREATE INDEX IF NOT EXISTS idx_seller_applications_user ON seller_applications(user_id, created_at DESC);
        CREATE UNIQUE INDEX IF NOT EXISTS idx_seller_payouts_order ON seller_payouts(order_id);
        CREATE INDEX IF NOT EXISTS idx_seller_payouts_seller ON seller_payouts(seller_user_id, status);
    `);
}
            ...
            const tx = db.transaction(async () => {
                for (const item of items) {
                    await db.prepare(`
                        ...
                        ) VALUES (?, 'platform', NULL, 'published', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    `).run(
                        item.id || '',
                        'arc-raiders',
                        item.category || 'items',
...
async function backfillNormalizedCatalogModel(db) {
    ...
    const tx = db.transaction(async () => {
        for (const product of products) {
            ...
            `).run(
                product.catalog_source || 'platform',
