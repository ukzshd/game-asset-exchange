...
const LOT_MUTATION_RETRY_LIMIT = 5;

function getLotAvailableQuantity(lot) {
    return Math.max(0, (lot.available_quantity || 0) - (lot.reserved_quantity || 0));
}

async function reserveLotQuantity(db, lotId, quantity) {
    ...
    `).run((lot.reserved_quantity || 0) + quantity, lotId, lot.available_quantity || 0, lot.reserved_quantity || 0, quantity);
    return result.changes > 0;
}

async function allocateReservedLotQuantity(db, lotId, quantity) {
    const lot = await db.prepare(`
        SELECT id, available_quantity, reserved_quantity
        FROM inventory_lots
        WHERE id = ?
    ...
    if ((lot.reserved_quantity || 0) < quantity || (lot.available_quantity || 0) < quantity) {
        return false;
    }
    ...
    const result = await db.prepare(`
        ...
        WHERE id = ?
          ...
          AND available_quantity >= ?
    `).run(
        (lot.available_quantity || 0) - quantity,
        (lot.reserved_quantity || 0) - quantity,
        lotId,
...
async function allocateLotQuantity(db, lotId, quantity) {
    const lot = await db.prepare(`
        SELECT id, available_quantity, reserved_quantity
        FROM inventory_lots
        WHERE id = ?
    ...
    if (getLotAvailableQuantity(lot) < quantity) {
        return false;
    }
    ...
    const result = await db.prepare(`
        ...
        WHERE id = ?
          ...
          AND reserved_quantity = ?
          AND (available_quantity - reserved_quantity) >= ?
    `).run((lot.available_quantity || 0) - quantity, lotId, lot.available_quantity || 0, lot.reserved_quantity || 0, quantity);
    return result.changes > 0;
}
    ...
    for (const item of items) {
        ...
        while (remaining > 0) {
            ...
            for (const lot of lots) {
                ...
                const updated = await reserveLotQuantity(db, lot.id, reserved);
                if (!updated) {
                    continue;
                }
            ...
            if (!progressed) {
                attempts += 1;
                if (attempts >= LOT_MUTATION_RETRY_LIMIT) {
                    throw new Error(`Insufficient inventory lots for product ${item.product_id}`);
                }
            } else {
                attempts = 0;
...
export async function decrementInventoryForOrder(db, orderId) {
    ...
    if (reservations.length > 0) {
        const touched = new Map();
        for (const reservation of reservations) {
            const updated = await allocateReservedLotQuantity(db, reservation.lot_id, reservation.quantity);
            if (!updated) {
    ...
    for (const item of items) {
        ...
        while (remaining > 0) {
            ...
            for (const lot of lots) {
                ...
                const updated = await allocateLotQuantity(db, lot.id, allocated);
                if (!updated) {
                    continue;
                }
