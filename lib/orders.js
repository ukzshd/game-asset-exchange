...
export function createOrderLog(db, {
    orderId,
    actorUserId = null,
    actorRole = 'system',
    eventType,
...
export function updateAffiliateStatusForOrder(db, orderId, status) {
    if (status === ORDER_STATUS.REFUNDED) {
        db.prepare("UPDATE affiliate_commissions SET status = 'cancelled' WHERE order_id = ?").run(orderId);
    }

    if (status === ORDER_STATUS.COMPLETED) {
        db.prepare("UPDATE affiliate_commissions SET status = 'paid' WHERE order_id = ?").run(orderId);
    }
}

export function setOrderStatus(db, {
    orderId,
    currentStatus,
    nextStatus,
    actorUserId = null,
    ...
    db.prepare(`
        ...
        SET status = ?,
            ...
            last_status_changed_at = datetime('now'),
    ...
    updateAffiliateStatusForOrder(db, orderId, nextStatus);
    createOrderLog(db, {
        orderId,
        actorUserId,
        actorRole,
...
export function assignOrder(db, {
    order,
    assignee,
    actor,
    note = '',
    ...
    db.prepare(`
        ...
        SET assigned_to = ?,
            ...
            status = ?,
            updated_at = datetime('now'),
            last_status_changed_at = CASE WHEN ? != status THEN datetime('now') ELSE last_status_changed_at END
        WHERE id = ?
