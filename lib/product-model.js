...
export async function syncNormalizedProductModel(db, productId, input) {
    ...
    const existingLot = await db.prepare('SELECT id FROM inventory_lots WHERE sku_id = ? LIMIT 1').get(skuId);
    if (existingLot) {
        await db.prepare(`
            UPDATE inventory_lots
            SET available_quantity = ?, updated_at = NOW()
            WHERE id = ?
        `).run(stockQuantity, existingLot.id);
    } else {
        await db.prepare(`
    ...
    await db.prepare(`
        UPDATE products
        SET spu_id = ?, sku_id = ?, stock_quantity = ?, updated_at = NOW()
        WHERE id = ?
    `).run(spuId, skuId, stockQuantity, productId);

    return { spuId, skuId, stockQuantity };
}
